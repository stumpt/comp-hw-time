...
Constants = np.matrix([[barConstant], [(barConstant + k)]])
...
while i < numElems:
    gStiff[i,i] += Constants[i]
    gStiff[i,(i+1)] += -Constants[i]
    gStiff[(i+1),i] += -Constants[i]
    gStiff[(i+1),(i+1)] += Constants[i]
    #iterate counter
    i+=1
...
springForce = (k*uFinal[1] - k*uFinal[2])*-1

while z < numElems:
    stress[z] = (E/L)*(u[(z+1)]-u[z])
    z+=1      
...
print("The nodal displacements, in [mm], ordered from lowest to highest numbered node are:\n")
print(uFinal)
print("\n")

print("Thus, the displacement of point B (node 2), in [mm], is given as:\n")
print(str(uFinal[1]))
...
print("The force in the spring, in [N], is:\n")
print(springForce)
print("\nNote: a (+) value indicates the element is in tension, while a (-) value in compression.\n")
