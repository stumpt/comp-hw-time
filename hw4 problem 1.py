...
#E (ksi), A (in2), L (in), t*b (kip/in)
E = 30000 #30E3
A = 2
L = 60 #5 ft
tb = 0.6 #600 lbf/in = 0.6 kip/in
constant = (0.6)/(60**2) #y=ax^2 -> a = constant = yx^-2 @ (60, 0.6)
...
for i in range(numElems):
    ...
    """
    tStar1 = lambda x: (constant*((x+(i*elementL))**2)) * (1 - (x/elementL))
    tStar2 = lambda x: (constant*((x+(i*elementL))**2)) * (x/elementL)
...
#make output pdf
pdf = PdfPages('Hw4_problem1_outputs.pdf')
...
for i in range(numElems):
    ...
    #plot labels
    plt.title("Problem 1: Displacement Field")
...
for i in range(numElems):
    ...
    #plot labels
    plt.title("Problem 1: Stress Field")
    plt.ylim(0, (np.max(stress))*1.1)
    plt.xlim(0, L)
    plt.ylabel("\u03C3 [ksi]")
    plt.xlabel("x [in]")
